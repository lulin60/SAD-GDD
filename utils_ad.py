...
def get_logp_norm(C, z, logdet_J):
    logp = C * _GCONST_ - 0.5*torch.sum(z**2, 1) + logdet_J
    logp = torch.exp(logp)
    return logp
...
def evaluate_thresholds_pix(gt_mask, scores): # lulin
    precision, recall, thresholds = precision_recall_curve(gt_mask, scores)
...
def convert_to_anomaly_max_scores_test(args, logps_list,size=None):
    normal_map = [list() for _ in range(args.FEATS_L)]
    for l in range(args.FEATS_L):
        ...
        if size==None:
            ...
            resize_ = args.IMG_SIZE
        normal_map[l] = F.interpolate(probs.unsqueeze(1),
            size=(resize_,resize_), mode='bilinear', align_corners=True).squeeze()

    # score aggregation
    scores = torch.zeros_like(normal_map[0])
    for l in range(args.FEATS_L):
        scores += normal_map[l]

    # normality score to anomaly score
    #scores = scores.max() - scores 
    top_values, top_indices = torch.topk(scores.view(scores.size(0), -1), k=196, dim=1)  # 在[C, H]维度上找到最大值
    mask_max = torch.zeros_like(scores)
    for i in range(scores.size(0)):
        indices = top_indices[i]
        row = indices // scores.size(2)  # Calculate row index
        col = indices % scores.size(2)  # Calculate column index
        mask_max[i, row, col] = True

    scores = scores * mask_max
    return scores


def convert_to_logits(args, logps_list,size=None):
    normal_map = [list() for _ in range(args.FEATS_L)]
    for l in range(args.FEATS_L):
        logps = torch.cat(logps_list[l], dim=0)  
        probs = logps
...
def convert_to_image_loglogits(args, logps_list,size=None):
    normal_map = [list() for _ in range(args.FEATS_L)]
    for l in range(args.FEATS_L):
        logps = torch.cat(logps_list[l], dim=0)  
        #logps-= torch.max(logps) # normalize log-likelihoods to (-Inf:0] by subtracting a constant
        ...
        if size==None:
            ...
            resize_ = args.IMG_SIZE
        normal_map[l] = F.interpolate(logps.unsqueeze(1),
            size=(resize_,resize_), mode='bilinear', align_corners=True).squeeze()

    # score aggregation
    logps_all = torch.zeros_like(normal_map[0])
    for l in range(args.FEATS_L):
        logps_all += normal_map[l]
...
def convert_to_anomaly_scores(args, logps_list,size=None):
    normal_map = [list() for _ in range(args.FEATS_L)]
    for l in range(args.FEATS_L):
        logps = torch.cat(logps_list[l], dim=0)  
        logps-= torch.max(logps) # normalize log-likelihoods to (-Inf:0] by subtracting a constant
