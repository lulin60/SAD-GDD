...
def SupConLoss(features, labels=None, mask=None,temperature=0.1, contrast_mode='all',\
    ...
    #logits_log = torch.log(exp_logits_sum)
    #max_l = max(logits_log.shape[0],1000)
    #print(torch.min(logits).item(),torch.min(logits_log).item(),torch.max(logits).item(),torch.max(logits).item(),'min2 max2 ****')
    #log_prob = logits[0:max_l,0:max_l] - logits_log[0:max_l,:]
